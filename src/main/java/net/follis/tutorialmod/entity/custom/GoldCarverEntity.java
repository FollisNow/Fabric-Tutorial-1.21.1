...
public class GoldCarverEntity extends PathAwareEntity implements InventoryOwner{
    ...
    private static final int craftCooldown = 25;
    ...
    private static final TrackedData<Integer> DATA_CURRENT_STATE =
            DataTracker.registerData(GoldCarverEntity.class, TrackedDataHandlerRegistry.INTEGER);

    public GoldCarverEntity(EntityType<? extends PathAwareEntity> entityType, World world) {
        ...
        this.goalSelector.add(2, new MoveToTargetPosGoal(this, 1.0F, 12, 3) {
            ...
            protected boolean isTargetPos(WorldView world, BlockPos pos) {
                if (world instanceof ServerWorld serverWorld) {
                    ...
                    return canGather && findIfPosHasItem(serverWorld, pos, ModTags.Items.GOLD_BLOCKS);
                }
                return false;
            }
            @Override
    ...
    private void setupStates() {
        ...
        int pickAmount = 1;
        if (Block.getBlockFromItem(inventoryStack.getItem()) instanceof SlabBlock) {
            pickAmount = 2;
        }
        ...
        switch (this.getCurrentState()) {
            ...
            case CRAFT -> {
                ...
                } else if (this.craftAnimationTimeout <= 10 ) {
                    this.getInventory().removeStack(0, pickAmount);

                    int dropAmount = 1;
                    if (Block.getBlockFromItem(this.getMainHandStack().getItem()) instanceof SlabBlock) {
                        dropAmount = 2;
                    }
    ...
    public static void playThrowSound(LivingEntity entity, ItemStack stack, Vec3d target) {
        ...
        world.playSoundFromEntity(null, entity, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundCategory.NEUTRAL, 0.8F, 1.5f);
    }
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        ItemStack itemStackSelf = this.getStackInHand(Hand.MAIN_HAND);
         if (itemStackSelf.isEmpty() && !itemStackPlayer.isEmpty() && itemStackPlayer.isIn(ModTags.Items.GOLD_BLOCKS)) {
    ...
    private boolean findIfPosHasItem(ServerWorld world, BlockPos pos, TagKey<Item> tagKey) {
        ...
        for (ItemEntity item : world.getEntitiesByClass(ItemEntity.class, Box.from(Vec3d.ofCenter(pos)), predicate)) {
            if (item != null) {
                return true;
            }
        }
